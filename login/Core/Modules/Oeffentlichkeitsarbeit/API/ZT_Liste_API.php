...
ini_set('error_log', __DIR__ . '/LI_Liste_API_php-error.log.txt');
...
register_shutdown_function(function() {
    ...
    if ($error !== null) {
        ...
        file_put_contents(__DIR__ . '/LI_Liste_fatal_error.log', $message, FILE_APPEND);
    }
});
...
use Fharch\Core\Modules\Oeffentlichkeitsarbeit\API\ZT_ListRepository;
use Fharch\Core\Modules\Oeffentlichkeitsarbeit\API\ZT_ListTableConfig;
...
try {
    ...
    $pdo = $dbLogger->getPDO();
    $repo = new ZT_ListRepository($pdo);
    ...
    $data = $repo->getZeitungen($listType, $search);
    #error_log('Search '. var_export($search, true));
    #error_log('ListType '. var_export($listType, true));
    $columns = ZT_ListTableConfig::getColumns($listType, $pdo);
