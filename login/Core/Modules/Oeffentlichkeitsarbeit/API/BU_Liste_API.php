...
ini_set('error_log', __DIR__ . '/BU_Liste_API_php-error.log.txt');
...
register_shutdown_function(function() {
    ...
    if ($error !== null) {
        ...
        file_put_contents(__DIR__ . '/MP_Liste_fatal_error.log', $message, FILE_APPEND);
    }
});
...
use Fharch\Core\Modules\Oeffentlichkeitsarbeit\API\BU_ListRepository;
use Fharch\Core\Modules\Oeffentlichkeitsarbeit\API\BU_ListTableConfig;
...
try {
    ...
    $pdo = $dbLogger->getPDO();
    $repo = new BU_ListRepository($pdo);
    ...
    $data = $repo->getBuecher($listType, $search);
    # error_log('Search '. var_export($search, true));
    # error_log('ListType '. var_export($listType, true));
    $columns = BU_ListTableConfig::getColumns($listType, $pdo);
