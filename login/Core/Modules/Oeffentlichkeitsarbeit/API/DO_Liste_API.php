...
register_shutdown_function(function() {
    ...
    if ($error !== null) {
        ...
        file_put_contents(__DIR__ . '/DO_Liste_fatal_error.log', $message, FILE_APPEND);
    }
});
...
use Fharch\Core\Modules\Oeffentlichkeitsarbeit\API\DO_ListRepository;
use Fharch\Core\Modules\Oeffentlichkeitsarbeit\API\DO_ListTableConfig;
...
try {
    ...
    $repo = new DO_ListRepository($pdo);
    
    $data = $repo->getDoku($listType, $search);
    #error_log('Search '. var_export($search, true));
    #error_log('ListType '. var_export($listType, true));
    $columns = DO_ListTableConfig::getColumns($listType, $pdo);
