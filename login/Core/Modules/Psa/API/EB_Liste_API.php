...
ini_set('error_log', __DIR__ . '../../../../login/AOrd_Verz/Logging/EL_Liste_API_php-error.log.txt');
...
register_shutdown_function(function() {
    ...
    if ($error !== null) {
        ...
        file_put_contents(__DIR__ . '../../../../login/AOrd_Verz/Logging//EL_Liste_fatal_error.log', $message, FILE_APPEND);
    }
});
...
use Fharch\Core\Modules\Psa\API\EL_ListRepository;
use Fharch\Core\Modules\Psa\API\EL_ListTableConfig;

require_once 'EL_ListRepository.php';
require_once 'EL_ListTableConfig.php';
...
try {
    ...
    $repo = new EL_ListRepository($pdo);
  
    
    // Debug-Ausgabe als Log, nicht als EL_dump
    # error_log("Repo Objekt: " . print_r($repo, true));
    
    // Parameter aus GET oder POST
    $listType = $_GET['T_List'] ?? 'Alle';
    $search = $_GET['search'] ?? null;
    $proj = $_GET['proj_ID'] ?? 'EHRG';  // proj = EHRG | AERM
    ...
    $columns = EL_ListTableConfig::getColumns($listType, $pdo);
