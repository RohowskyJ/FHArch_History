 ...
 *
 */
class AWA_ListRepository {
    private PDO $pdo;
    protected static string $logFile = '../../../../login/AOrd_Verz/Logging/AWA_ListRepository_debug.log.txt';
    /*
    ...
    */
    ...
    public function getAermAbz(string $listType, string $fo_fw_id, ?string $search = null) : array {
        $sql = "SELECT * FROM psw_aermel_abz ";
        $where = [];
        $params = [];
    ...
    protected function modifyRow(array &$row, string $tabTyp): bool
    {
        ...
        if ($row['fo_ff_abzeich'] != "") {
            ...
            if ($graffile) {
                #self::log("Bild");
                $row['fo_ff_abzeich'] = "<a href='$d_path$bild1' target='_blank'><img src='$d_path$bild1' alt='$bild1' height='200' ></a>";
            } else {
                $row['fo_ff_abzeich'] = "<a href='$d_path$bild1' target='_blank'>$bild1</a>";
            }
        }
     
        // Unnötige Felder entfernen
        unset($row['fo_ff_a_sort'], $row['fo_ff_a_typ_a'], $row['fo_changed_id'], $row['fo_changed_at']);
