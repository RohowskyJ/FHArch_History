...
ini_set('error_log', __DIR__ . '../../../../login/AOrd_Verz/Logging/AWF_Liste_API_php-error.log.txt');
...
register_shutdown_function(function() {
    ...
    if ($error !== null) {
        ...
        file_put_contents(__DIR__ . '/AWF_Liste_fatal_error.log', $message, FILE_APPEND);
    }
});
...
use Fharch\Core\Modules\Psa\API\AWF_ListRepository;
use Fharch\Core\Modules\Psa\API\AWF_ListTableConfig;

require_once 'AWF_ListRepository.php';
require_once 'AWF_ListTableConfig.php';
...
try {
    ...
    $repo = new AWF_ListRepository($pdo);
  
    
    // Debug-Ausgabe als Log, nicht als AWF_dump
    ...
    $data = $repo->getFfWapp($listType, $proj, $search);
    #error_log('Search '. var_export($search, true));
    #error_log('ListType '. var_export($listType, true));
    $columns = AWF_ListTableConfig::getColumns($listType, $pdo);
