...
final class DB_Psa
{
    ...
    /**
     * Hole Datensatz aus fwo_ort_ref nach fw_id
     */
    public function getOrtById(int $id): ?array
    {
        return $this->select('pso_ort_ref', ['*'], ['fw_id' => $id], [], 1)[0] ?? null;
    }
    
    /**
     * Beispiel: Füge neuen Datensatz in aw_ff_wappen ein
     */
    public function insertOrt(array $data): int
    {
        return $this->insert('pso_ort_ref', $data);
    }
    
    /** update von bestehendem Record */
    public function updateOrt(int $id, array $data): int
    {
        return $this->update('pso_ort_ref', $data, ['fw_id' => $id]);
    }
    
    /**
     * Beispiel: Hole Datensatz aus aw_aermel_abz nach fo_id
     */
    public function getAermelAbzByFw(int $id): ?array
    {
        return $this->select('psw_aermel_abz', ['*'], ['fo_fw_id' => $id], [], 1)[0] ?? null;
    }
    ...
    {
        return $this->insert('psw_ff_wappen', $data);
    }
...
use FSArch\Login\Tables\DB_Psa;
