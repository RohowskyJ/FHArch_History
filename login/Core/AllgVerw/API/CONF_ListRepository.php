 ...
 *
 */
class CONF_ListRepository {
    private PDO $pdo;
    protected static string $logFile = 'BenutzerRepository_debug.log.txt';
    ...
    public function getUsers(string $listType, ?string $search = null): array {
        ...
        $sql = "
            SELECT
                ...
                d.fd_tel,
                d.fd_email
            FROM fv_adm_mail as a
            LEFT JOIN fv_benutzer as b ON b.be_id = a.be_ids
    ...
    protected function modifyRow(array &$row, $tabTyp)
    {
        ...
        $em_id = $row['em_id'] ?? 0;
        $row['action'] = "<a href='AdmEmailEdit.php?ID={$em_id}'>Edit</a>";
