...
ini_set('error_log', __DIR__ . '/Err_StaatAutoComp_API_php-error.log.txt');
...
$logFile = $logPath . '/StaatenAutoComp_API.log';
...
use Fharch\Core\AllgVerw\API\StaatenAutocompleteAPI;

$debug = isset($_GET['debug']) && in_array(strtolower((string)$_GET['debug']), ['1', 'true', 'on', 'yes'], true);

// Autocomplete Handler für Staaten mit der neuen API-Klasse
$term = $_GET['term'] ?? '';
$term = trim($term);

if ($debug) {
    error_log('StaatenAutoComp_API debug enabled for term=' . $term . ' uri=' . ($_SERVER['REQUEST_URI'] ?? '')); 
}
...
try {
    $autocomplete = new StaatenAutocompleteAPI();
    $autocomplete->handleRequest();
} catch (Exception $e) {
    error_log('StaatenAutocompleteAPI Error: ' . $e->getMessage());
