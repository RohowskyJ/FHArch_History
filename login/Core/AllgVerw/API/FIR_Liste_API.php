...
ini_set('error_log', __DIR__ . '/FIR_API_php-error.log.txt');
...
register_shutdown_function(function() {
    ...
    if ($error !== null) {
        ...
        file_put_contents(__DIR__ . '/FIR_fatal_error.log', $message, FILE_APPEND);
    }
});
...
use Fharch\Core\AllgVerw\API\FIR_ListRepository;
use Fharch\Core\AllgVerw\API\FIR_ListTableConfig;
...
try {
    ...
    $pdo = $DBD->getPDO();
    $repo = new FIR_ListRepository($pdo);
    ...
    $columns = FIR_ListTableConfig::getColumns($listType, $pdo);
