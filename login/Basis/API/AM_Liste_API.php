...
if (!class_exists(\FSArch\Login\Basis\AM_ListeRepository::class)) {
    require_once __DIR__ . '/AM_ListeRepository_CLS.php';
}
if (!class_exists(\FSArch\Login\Basis\AM_ListeTableConfig::class)) {
    require_once __DIR__ . '/AM_ListeTableConfig_CLS.php';
}
...
try {
    ...
    $vfDatabase = \FSArch\Login\Basis\FS_Database::getInstance();
    $pdo = $vfDatabase->getPDO();
    $repo = new \FSArch\Login\Basis\AM_ListeRepository($pdo);
    ...
    $columns = \FSArch\Login\Basis\AM_ListeTableConfig::getColumns($listType, $pdo);
